...
@export var room_size = Vector2(50, 50)
...
func spawn_actual_rooms():
	var offset = Vector2((length / 2) * room_size.x, (length / 2) * room_size.y)
	...
	for x in range(length):
		for y in range(length):
			if map[x][y]:
				...
				newRoom.position = Vector2(x * room_size.x, y * room_size.y) - offset
